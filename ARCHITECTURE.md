...
`data/merged/` directory. Prefixes each filename with its source name to avoid
...
Deletes `data/merged/` after splitting to save storage (can be regenerated by re-running Stage 2).

### Stage 4 — `scripts/4_verify_dataset.py`
Sanity checks before training:
- Confirms every label file has a corresponding image (logs missing images)
- Validates YOLO format: 5 columns, class ID in [0, 5], coordinates in [0.0, 1.0]
- Reports per-class annotation counts in each split
...
- Per-class AP for all 6 classes
...
- `NUM_CLASSES = 20` with 6 actual classes → set `nc: 6` exactly
